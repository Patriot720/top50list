...
public class PlayerBackground extends AsyncTask<String,Void,List<Player>> {
    ...
    PlayerBackground(Context context, PlayerAdapter adapter, SwipeRefreshLayout layout){
    mDb = new PlayersDbHelper(context).getWritableDatabase();
    mAdapter = adapter;
    mRefreshLayout = layout;
    }
    PlayerBackground(Context context){
    ...
    public int updatePlayer(Player player) {
        ...
        int update = mDb.update(PlayersDbHelper.TABLE_NAME, cv, "username='" + player.username + "'", null);
        return update;
    }
}
